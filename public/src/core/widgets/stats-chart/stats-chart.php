...
foreach ($data as $row) {
    ...
    $dateIps[$date] = [];
    $cIndex = $date . '--Views';
    $context = json_decode($row[3], true);
    if ($filters) {
        foreach ($filters as $key => $value) {
            if (!isset($context[$key]) || $value != $context[$key]) {
                continue;
            }
        }
    }
    if ($dataGroup && ($context[$dataGroup] || !in_array($row[2], $dateIps[$date]))) {
        $dataLabel = $context[$dataGroup] ?? 'N/A';
