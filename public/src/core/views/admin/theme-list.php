...
foreach ($packages as $pkey => $p) {
    ...
    if (file_exists('themes/' . $p->package)) {
        if ($p->package != Config::get('theme')) {
            $table .= "<a onclick='theme_activate(\"{$p->package}\")' class='btn'>" . __('Select') . "</a> ";
        } elseif (Config::get('env') == 'dev') {
            $table .= "<a onclick='theme_activate(\"{$p->package}\")' class='btn btn-primary'><i class='fa fa-refresh'></i></a> ";
        }
        if (isset($p->options)) {
            $table .= "<a onclick='theme_options(\"{$p->package}\")' class='btn' style='display:inline-flex'><i class='fa fa-gears'></i>&nbsp;</a> ";
        }
        if (@$current_version = json_decode(file_get_contents('themes/' . $p->package . '/package.json'))->version) {
            if (version_compare($p->version, $current_version) > 0) {
                $table .= " <a onclick='theme_download(\"{$p->package}\")' class='btn btn-success'>" . __('Upgrade') . "</a> ";
            }
        }
        $table .= "<a href='" . Config::base() . "?g_preview_theme={$p->package}' target='_blank' class='btn' style='display:inline-flex'><i class='fa fa-eye'></i>&nbsp;</a> ";
        if (FS_ACCESS) {
            $table .= "<a href='admin/fm/?f=themes/{$p->package}' target=\"_blank\" class='btn'><i class=\"fa fa-folder\"></i></a>";
        }
    } else {
        $table .= "<a onclick='theme_download(\"{$p->package}\")' class='btn btn-success'>" . __('Download') . "</a>";
    }
    if (isset($p->parent)) {
...
<?=View::script('core/admin/menu-editor.js');?>
<?=View::script('core/admin/color-input303.js');?>
<script>
