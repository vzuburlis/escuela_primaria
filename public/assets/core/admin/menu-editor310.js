...
<td>\
  ...
  <span v-if="typeof types[row.type]!=\'undefined\'">\
    ...
    <option v-if="types[row.type]" v-for="(option,i) in types[row.type].options" :value="i">{{option}}</option>\
    </select></span>\
...
',
  ...
  methods:{
    ...
    displayType: function(type) {
      ...
      if(type=='dir') return g.tr('Dropdown menu', {'es':'Menú desplegable'})
      if(type=='btn') return g.tr('Button A', {'es':'Botón A'})
      if(type=='btn2') return g.tr('Button B', {'es':'Botón B'})
      if (this.types[type] && this.types[type].label) {
