...
class BlogController extends Controller
{
  ...
  */
    public function indexAction()
    {
        ...
        if ($path == 'blog' && Page::inCachedList('blog')) {
            $this->postShow('blog');
            return;
        }
        if (self::$page > 1) {
  ...
  */
    public function feedAction()
    {
        ...
        View::set('description', Config::get('description')??Config::get('slogan'));
  ...
  */
    public function category_GET($category = 0)
    {
        ...
        $name = DB::value("SELECT title from postcategory WHERE id=?", $category ?? 0);
        ...
        $totalpages = max(ceil((Post::total($args)) / 12), 1);
        $posts = Post::getPosts($args);
        if (self::$page < 1 || self::$page > self::totalPages()) {
            http_response_code(404);
            Cache::page('404_c' . Config::lang(), 86400);
            View::render('404.php');
            return;
        }
        View::set('category_name', $name);
  ...
  */
    ...
    {
        $cacheTime = Config::get('blog.cache');
        ...
        if (Post::inCachedList($postId) && ($r = Post::fetchByIdSlug($postId)) && ($r['publish'] == 1)) {
            if (!empty($r['language'])) {
            ...
            View::set('post', $r);
            View::set('categories', Post::categories($r['id']));
            ...
            View::meta('og:publish_date', $r['updated']);

            if (empty($r['img']) && Config::get('og-image')) {
                $r['img'] = Config::get('og-image');
            }
            ...
            if ($value = Config::get('blog.twitter_card')) {
                View::meta('twitter:card', $value);
            }
            if ($value = Config::get('blog.twitter_site')) {
                View::meta('twitter:site', '@' . $value);
            }

            if ($r['user'] || $r['user'] = User::getById($user_id)) {
                View::set('author', $r['user']['username']);
                View::meta('author', $r['user']['username']);
