  ...
  <li :id="uid" data-index="index">
      <div @dragstart="dragStart($event)" @dragend="$emit('remove')" draggable="true">
          ...
          <span v-if="model.type=='menu'" style="float:right">
            ...
            &nbsp;<a @click="deleteMenu" class="btn-sm btn-error"><i class="fa fa-trash"></i></a>
          </span>
...
<div class="row">
  ...
  <div id="menu-list" class="gm-3 wrapper">
    ...
    <?php
    $menus = array_merge(Menu::getList());
    ...
    <div>
      ...
      <button class="g-btn fullwidth" onclick="window.location.href='<?=Config::url('admin/menu/')?>'+g('#new-menu').all[0].value"><?=__("New")?></button>
    </div>
  </div>
</div>
