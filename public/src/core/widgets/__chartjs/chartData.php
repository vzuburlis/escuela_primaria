<?php

global $chartColors;

$chartColors = [
    'rgb(255, 99, 132)',
    'rgb(75, 192, 192)',
    'rgb(255, 159, 64)',
    'rgb(255, 205, 86)',
    'rgb(54, 162, 235)',
    'rgb(153, 102, 255)',
    'rgb(201, 203, 207)',
    'rgb(255, 109, 142)',
    'rgb(85, 202, 202)',
    'rgb(255, 169, 74)',
    'rgb(255, 215, 96)',
    'rgb(74, 162, 235)',
];
if ($locale = Config::get('locale') ?? Config::lang()) {
    setlocale(LC_ALL, $locale);
}
if ($palette = Config::get('admin_palette')) {
    $palettes = json_decode($palette, true);
    $chartColors = [];
    foreach ($palettes as $p) {
        $chartColors[] = 'rgb(' . hexdec($p[1] . $p[2]) . ',' . hexdec($p[3] . $p[4]) . ',' . hexdec($p[5] . $p[6]) . ')';
    }
    foreach ($palettes as $p) {
        $chartColors[] = 'rgba(' . hexdec($p[1] . $p[2]) . ',' . hexdec($p[3] . $p[4]) . ',' . hexdec($p[5] . $p[6]) . ',0.7)';
    }
}

if ($data['data'] == 'array') {
    foreach ($data['chartData']['datasets'] as $i => $ds) {
        if (!isset($ds['backgroundColor'])) {
            $data['chartData']['datasets'][$i]['backgroundColor'] = $chartColors;
        }
    }
    return $data['chartData'];
}

if ($data['query'] != '') {
    $labels = [];
    $datasets = [];
    $chartData = [];
    $label_data = [];
    $labelsN = 0;
    $datasetsN = 0;
    $chartData['datasets'] = [];
    $dataArray = [];

    if ($data['data'] == 'mysql') {
        try {
            $res = Gila\DB::query($data['query']);
        } catch (\Exception $e) {
        }

        if ($res) {
            while ($r = mysqli_fetch_row($res)) {
                if (strlen($r[0]) > 22) {
                    $r[0] = substr($r[0], 0, 22);
                }
                $dataArray[] = $r;
            }
        } elseif ($error = Gila\DB::error()) {
            echo "<div class=\"alert alert-warning\">Mysql error: $error</div>";
        }
    } elseif ($data['data'] == 'json') {
        return json_decode($data['query'], true);
    } else {
        $dataRows = explode("\n", $data['query']);
        foreach ($dataRows as $r) {
            $dataArray[] = str_getcsv($r);
        }
    }

  // fill label dates
    if (isset($data['labelDates'])) {
        $start = $data['labelDates'][0] ?? date('Y-m-d', strtotime('-' . ($data['labelDates'] - 1) . ' days'));
        $end = $data['labelDates'][1] ?? date('Y-m-d');
        $range = [$start];
        $i = 0;
        do {
            $i++;
            $newDate = date('Y-m-d', strtotime($start . ' +' . $i . ' days'));
            $range[] = $newDate;
        } while ($newDate != $end);

        foreach ($range as $cdate) {
            $dateFound = false;
            foreach ($dataArray as $r) {
                if ($r[0] == $cdate) {
                          $dateFound = true;
                }
            }
            if ($dateFound == false) {
                foreach ($dataArray as $i => $r) {
                    if ($r[0] > $cdate) {
                              array_splice($dataArray, $i, 0, [[$cdate, $data['fillDataset'] ?? '', 0]]);
                              $dateFound = true;
                              break;
                    }
                }
            }
            if ($dateFound == false) {
                $dataArray[] = [$cdate, $data['fillDataset'] ?? '', 0];
            }
        }
    }

  // fill label months
    if (isset($data['labelMonths'])) {
        $start = $data['labelMonths'][0] ?? date('Y-m', strtotime('-' . ($data['labelMonths'] - 1) . ' months'));
        $end = $data['labelMonths'][1] ?? date('Y-m');
        $range = [$start];
        $i = 0;
        do {
            $i++;
            $newDate = date('Y-m', strtotime($start . ' +' . $i . ' months'));
            $range[] = $newDate;
        } while ($newDate != $end);

        foreach ($range as $cdate) {
            $dateFound = false;
            foreach ($dataArray as $r) {
                if ($r[0] == $cdate) {
                          $dateFound = true;
                }
            }
            if ($dateFound == false) {
                foreach ($dataArray as $i => $r) {
                    if ($r[0] > $cdate) {
                              array_splice($dataArray, $i, 0, [[$cdate, $data['fillDataset'] ?? '', 0]]);
                              $dateFound = true;
                              break;
                    }
                }
            }
            if ($dateFound == false) {
                $dataArray[] = [$cdate, $data['fillDataset'] ?? '', 0];
            }
        }
    }

  // change labels date format
    if (isset($data['labelDateFormat'])) {
        foreach ($dataArray as $i => $r) {
            $dataArray[$i][0] = date_format(date_create($r[0]), $data['labelDateFormat']);
        }
    }
    if (isset($data['labelMonthFormat'])) {
        foreach ($dataArray as $i => $r) {
              //$dateObj = new DateTime();
              //$dataArray[$i][0] = $dateObj->setTimestamp(strtotime($r[0]))->format($data['labelMonthFormat']);
            $dataArray[$i][0] = View::date($r[0], $data['labelMonthFormat']);
        }
    }
    if (isset($data['labelWeekFormat'])) {
        foreach ($dataArray as $i => $r) {
            $dateObj   = DateTime::createFromFormat('!w', $r[0]);
            $week_start = new DateTime();
            $week_start->setISODate(date('Y'), $r[0]);
            $dataArray[$i][0] = date_format($week_start, $data['labelWeekFormat']);
        }
    }

    foreach ($dataArray as $r) {
        if (!in_array($r[0], $chartData['labels'] ?? [])) {
            $chartData['labels'][] = $r[0];
        }
        if (!in_array($r[1], $datasets)) {
            $datasets[] = $r[1];
            $chartData['datasets'][] = [
            'label' => $r[1],
            'borderWidth' => 2,
            'fill' => false,
            'backgroundColor' => $chartColors[$datasetsN],
            'borderColor' => $chartColors[$datasetsN],
            ];
            $datasetsN++;
        }
        @$label_data[$r[0]][$r[1]] = $r[2];
    }


    foreach ($chartData['labels'] as $ilabel => $label) {
        foreach ($chartData['datasets'] as $ids => $ds) {
            if ($v = @$label_data[$label][$ds['label']]) {
                @$chartData['datasets'][$ids]['data'][$ilabel] = $v;
            } else {
                @$chartData['datasets'][$ids]['data'][$ilabel] = null;
            }
        }
    }

    return $chartData;
} else {
    return [
    'labels' => ['January', 'February', 'March', 'April', 'May', 'June', 'July'],
    'datasets' => [
      ['label' => 'Dataset 1','backgroundColor' => 'rgb(255, 99, 132)','borderWidth' => 0,
      'data' => [11, 8, 16, 2, 15, 13, 10]],
      ['label' => 'Dataset 2','backgroundColor' => 'rgb(54, 162, 235)','borderWidth' => 0,
      'data' => [11, 13, 14, 20, 5, 9, 11]]
    ]
    ];
}
