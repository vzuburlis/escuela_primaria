            ...
            if (
                ...
                if (isset($pac['require'])) {
                    foreach ($pac['require'] as $key => $value) {
                        if (!Config::inPackages($key) && $key != 'core') {
                            $require[$key] = $key . ' v' . $value;
                        } else {
                            $pacx = json_decode(file_get_contents(Config::src($key) . '/package.json'), true);
  ...
  */
    public static function download($download)
    {
        if ($download) {
            ...
            $request = 'https://pyme.one/packages/themes?theme=' . $download;
            $pinfo = json_decode(file_get_contents($request), true)[0];

            if (!$pinfo) {
                Response::error(__('_theme_not_downloaded'));
            }
            $file = 'https://pyme.one/assets/themes/' . $download . '.zip';
