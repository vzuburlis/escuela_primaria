...
class Profile
{
    public static function postUpdate($user_id)
    {
        ...
        if (Request::post('submit-btn') === 'submited') {
            ...
            if ($_POST['gila_photo'] == null) {
                ...
                $path = strip_tags($_POST['gila_photo']);
                if ($user_id == Session::userId()) {
                    Session::key('user_photo', $path);
                }
                User::meta($user_id, 'photo', $path);
            }
            if (isset($_POST['user_language'])) {
                DB::query("UPDATE user SET `language`=? WHERE id=?;", [$_POST['user_language'], $user_id]);
                if ($user_id == Session::userId()) {
                    Session::key('language', $_POST['user_language']);
                    Config::lang($_POST['user_language']);
                    Config::loadLang('core/lang/myprofile/');
                    Config::loadLang('core/lang/admin/');
    ...
    public static function generateToken()
    {
        ...
        } while (DB::value("SELECT COUNT(*) FROM sessions WHERE gsessionid=?;", $token) > 0);
        return $token;
    }
    ...
    {
        $permissions = [];
        $packages = array_merge(Config::packages(), ['core','app']);
        foreach ($packages as $package) {
            $pjson = Config::src($package) . '/package.json';
            $perjson = Config::src($package) . '/package/en.json';
