...
if ($data['src'] == 'demo') {
    ...
    $pages = DB::get("SELECT title, description, image FROM `page` WHERE publish=1 ORDER BY id DESC LIMIT ?", [$data['items-to-show']]);
}
?>
<?=View::script('lib/bootstrap52/bootstrap.bundle.min.js')?>
<?=View::cssAsync('lib/bootstrap52/bootstrap.min.css')?>
<div id="MyPageCarousel" class="carousel slide my-5 carousel-dark <?= ($data['carousel-full-width'] == '1') ? 'position-absolute w-100 start-0' : '' ?>" data-bs-ride="carousel">
  <div class="carousel-indicators">
    <?php if ($pages) {
        foreach ($pages as $i => $page) { ?>
        <button type="button" data-bs-target="#MyPageCarousel" data-bs-slide-to="<?=$i?>" <?= $i == 0 ? 'class="active"' : '' ?> aria-current="true" aria-label="Slide <?=$i + 1?>"></button>
        <?php }
    }?>
  </div>
    ...
    <?php if ($pages) {
        ...
        <div class="carousel-item <?= $i == 0 ? 'active' : '' ?>">
          ...
          <div class="carousel-caption d-none d-md-block <?=( isset($data['text-align']) ? 'text-' . $data['text-align'] : '') ?>"  style="height: <?=( isset($data['vertical-align']) ? (($data['vertical-align']) * 10) . '%' : '') ?>">
              ...
              <h2 class="h2" style="color:<?=$data['color-text'] ?? 'white'?>"><?= $page['title'] ?></h2>
              <?php } ?>
              <?php if (isset($page['description'])) { ?>
              <p class="h4" style="color:<?=$data['color-text'] ?? 'white'?>"><?= $page['description'] ?></p>
              <?php } ?>
