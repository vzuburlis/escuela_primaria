...
class Package
{
    ...
    public static function config($package)
    {
        if (!isset(self::$config[$package])) {
            $path = Config::src($package) . '/package.json';
    ...
    public static function display($package)
    {
        ...
        echo Cache::remember($itemName, 259200, function ($u) {
            return file_get_contents("https://pyme.one/addons/package/{$u[0]}/html");
  ...
  */
    public static function activate($activate)
    {
        ...
        if (in_array($activate, scandir(Config::src()))) {
            if (
                !in_array($activate, $packages) ||
                Config::get('env') == 'dev'
            ) {
                $pac = json_decode(file_get_contents(Config::src($activate) . '/package.json'), true);
                if (!FS_ACCESS && isset($pac['mainsite']) && $pac['mainsite'] === true) {
                    Response::error('Package for main site');
                }
                $require = [];
                $require_op = [];
                ...
                if ($require === [] && $require_op === []) {
                    ...
                    if ($require != []) {
                        ...
                        foreach ($require as $k => $r) {
                            if (strpos($k, '/')) {
                          //$error .= "<br><a href='https://pyme.one/blog/36' target='_blank'>$k $r</a>";
                            } else {
        ...
        if (isset($error)) {
            ...
            Log::debug(Session::key('user_email') . " activated package " . $activate);
            Response::success(['reload' => $pac['require_refresh'] ?? true]);
        }
    }
        ...
        if (in_array($deactivate, $packages)) {
          ...
          // deactivate other packages that require $deactivate
            foreach (Config::packages() as $p) {
                $string = file_get_contents(Config::src() . "/$p/package.json");
  ...
  */
    public static function download($package)
    {
        ...
        $zip = new \ZipArchive();
        $target = Config::src($package);
        $request = 'https://pyme.one/packages/?package=' . $package;
        ...
        $localfile = Config::src($package) . '.zip';
    ...
    public static function update($package)
    {
        ...
        Table::$options = false; //shall work for all packages
        $update_file = Config::src($package) . '/update.php';
        self::addRoles($package);
        ...
        Event::fire('Package::update', ['package' => $package]);
    }
    ...
    public static function updateSites($package)
    {
        ...
        $sites = scandir('sites');
        $update_file = Config::src($package) . '/update.php';
        $default_conn = $GLOBALS['config']['db'];
        $schemas = DB::getList("SHOW DATABASES;");
        foreach ($sites as $site) {
            if ($site[0] != '.') {
                ...
                if (file_exists($config) && file_exists($update_file)) {
                    ...
                    // skip if database dont exist
                    if (!in_array($dbname, $schemas)) {
                        continue;
                    }
                    $packages = $GLOBALS['config']['packages'];
                    $txt = DB::value("SELECT `value` FROM $dbname.option WHERE `option`='packages';");
                    if ($txt) {
                        $packages = array_unique(array_merge($packages, json_decode($txt, true)));
                    }
                    if (
                        $package === 'core' ||
    ...
    {
        $assets = Config::src($package) . '/assets';
  ...
  */
    ...
    {
        if (file_exists(Config::src($package))) {
            echo '<form id="addon_options_form" class="container" ';
            echo 'style="display:grid;gap:1em;grid-template-columns:repeat(auto-fill,minmax(320px,1fr))">';
            echo '<input id="addon_id" value="' . $package . '" type="hidden">';
            $pack = $package;
            if (file_exists(Config::src($package) . '/package.json')) {
                $pac = json_decode(file_get_contents(Config::src($package) . '/package.json'), true);
                @$options = $pac['options'];
            } else {
                die('Could not find ' . Config::src($package) . '/package.json');
            }

            if (is_array($options)) {
                Config::loadOptions();
                foreach ($options as $key => $op) {
                    $values[$key] = Config::get($pack . '.' . $key);
                }
  ...
  */
    ...
    {
        $updateConfig = false;
        $jsonFile = Config::src($package) . '/package.json';
        if (file_exists($jsonFile)) {
            ...
            foreach ($_POST['option'] as $key => $value) {
                if (isset($data['options'][$key])) {
                    ...
                    if ($data['options'][$key]['secret'] ?? false) {
                        $GLOBALS['config'][$package . '.' . $key] = $value;
                        $updateConfig = true;
                    } else {
            ...
            if ($updateConfig) {
                ...
                @rename(CONFIG_PHP, CONFIG_PHP . '_prv');
  ...
  */
    public static function updateLoadFile()
    {
        ...
        foreach ($packages as $package) {
            $handle = @fopen(Config::src() . "/$package/load.php", "r");
            if ($handle) {
                $line = fgets($handle);
                $contents .= "\n// $package\n";
        ...
        DB::close();
        $options = "Gila\Config::\$option = " . var_export(Config::$option, true) . ";\n";
        $options .= "Gila\Config::loadEnv();\n\n";
    ...
    {
        return false;
        if (!FS_ACCESS || Config::get('check4updates') == 0) {
            return false;
        }
      ...
      // check once a day
        if ($diff > 0) {
            ...
            $packages2update = [];
            $versions = [];
          //$url = "https://pyme.one/addons/package_versions?p[]=".implode('&p[]=', array_keys($installed_packages));
    ...
    public static function addRoles($package)
    {
        $file = Config::src($package) . '/package.json';
        $permissions = $GLOBALS['config']['permissions'];
        if (file_exists($file)) {
            $pac = json_decode(file_get_contents($file), true);
            $pac_roles = $pac['roles'][Config::lang() ?? 'en'] ?? [];
            foreach ($pac_roles as $new_role) {
                $role_exists = false;
                foreach ($permissions as $role_id => $role_per) {
                    if ($per == [$per]) {
                        $role_exists = true;
                    }
                }
                if ($role_exists == false && !DB::value("SELECT id FROM userrole WHERE userrole=?", [$new_role[0]])) {
                    DB::query("INSERT INTO userrole(userrole,`level`,`description`) VALUES(?,?,?)", [
                    $new_role[0], $new_role[1], $new_role[2],
                    ]);
                    $permissions[DB::$insert_id] = [$per];
                    Config::set('permissions', $permissions);
                    if (isset($pac['options'])) {
                        foreach ($pac['options'] as $on => $op) {
                            if (isset($op['type']) && $op['type'] == 'admin_role' && isset($op['permission'])) {
                                if (!Config::get($package . '.' . $on)) {
                                    Config::set($package . '.' . $on, DB::$insert_id);
                                }
                            }
                        }
                    }
